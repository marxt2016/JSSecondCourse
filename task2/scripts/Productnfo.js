...
function createProductCardMarkup(product) {
    ...
    <div class="featured_item">
             <a class="image_featured" href="#">
                <img class="image_item" src="img/${product.img}" alt="product">
            ...
            <div class="to_cart" data-prod_id ="${product.id}">
...
productList.insertCardItems(productList.products, featuredItemElement);

/**
 * EventListener on 'click' for all elements "Add to Cart" for each Product Item found on the page.
 * AFinally calls function addToShopping cart from Shoppingcart.js to create shopping cart markup.
 */

function AddEventListenerOnClickAddToCart() {
    let allAddToCartElements = document.querySelectorAll('.to_cart');
    allAddToCartElements.forEach(function (element) {
        element.addEventListener('click', function (event) {
            const prodID = event.currentTarget.getAttribute('data-prod_id');
            addProductToShoppingcart(prodID);
        })
    })
}
AddEventListenerOnClickAddToCart();
